...
node("nodejs"){
    ...
    stage('Clone Git Repo') {
        catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
            git branch: 'main', credentialsId: 'github-goitProjects', url: 'git@github.com:goitProjects/callboard_js_backend.git'
        }
    }
    ...
    stage('Copy files'){
        ...
        if (success) {
            catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                ...
                sh "mkdir -p /root/slave_jenkins/workspace/Build-Callboard-backend/src/gcp-config"
                //Copy files for Build stage
                sh '''cp /root/keys/index.ts /root/slave_jenkins/workspace/Build-Callboard-backend/src/gcp-config/index.ts'''
                sh "cp /root/keys/keys.json /root/slave_jenkins/workspace/Build-Callboard-backend/src/gcp-config/keys.json"
                sh "cp /root/keys/callboard-backend/.env /root/slave_jenkins/workspace/Build-Callboard-backend/.env"
            }
        }
    }
    ...
    stage('Deploy') {
        ...
        if (success) {
            catchError(buildResult: 'FAILURE', stageResult: 'FAILURE') {
                ...
                sh "scp -r ./* ${env.sshUserAndHost}:/root/callboard-backend"
            }
        }
    }
